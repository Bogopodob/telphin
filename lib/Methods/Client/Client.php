...
trait Client
{
  function getClSoundDownload($client_id = "@me", $sound_id)
  {
    $path = self::$version . "/client/{$client_id}/sound/{$sound_id}/download/";
    $method = "GET";
    $result = $this->makeRequest($path, $method, [], false, true);
    return $result;
  }

  function getClSounds($client_id = "@me", $data = [])
  {
    $path = self::$version."/client/{$client_id}/sound/";
    $method = "GET";
